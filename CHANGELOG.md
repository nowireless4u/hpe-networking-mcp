...
### Added
- **EdgeConnect platform** (`edgeconnect_*`). 1216 tools across 187 tag-derived modules under `platforms/edgeconnect/tools/`, generated by `scripts/generate_edgeconnect_tools.py` from `vendor/edgeconnect/EdgeConnect-9-7-REST-API.json`. Tool names derive from HTTP method + URL path (`edgeconnect_<method>_<path-slug>`) because the spec's operationIds are unusable (687/1216 are auto-numbered, 12 collide); the 2 residual slug collisions are de-duped with a numeric suffix.
