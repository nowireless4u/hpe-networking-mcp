...
**Minor — credentials may now come from a `.env` file or environment variables, alongside the existing Docker secret files.** Classified minor rather than patch because it changes how the server is configured and deployed. Existing secrets-based deployments are unaffected: the secret file still wins and the base `docker-compose.yml` still mounts it, so `docker compose up -d` keeps working unchanged.

### Added
- **`.env`-file / environment-variable credentials** (contributed by @michaelrosejr, #619). Credentials may now come from a `.env` file / environment variables *in addition to* Docker secret files: `_read_secret()` reads the secret file at `SECRETS_DIR/<name>` first (file wins) and falls back to the `NAME.upper()` env var. **Backward compatible:** the base `docker-compose.yml` keeps its Docker secret-file mounts, so existing secrets deployments run unchanged on the plain `docker compose up -d`. New `.env`-only deployments add the `docker-compose.env.yml` overlay (`docker compose -f docker-compose.yml -f docker-compose.env.yml up -d`), which drops the secret mounts so no `secrets/*` files are needed. Adds `.env.example` and the overlay. The default bind stays **loopback (`127.0.0.1`)** — expose to `0.0.0.0` only behind an authenticating proxy/firewall.

### Changed
- **Vendored specs refreshed** — HPE GreenLake OAS synced 2026-08-03 (#644) and Mist OpenAPI synced to 2607.1.0 (#643). Both are automated syncs; they change the specs behind tool bodies and `get_schema` enrichment, not the tool surface itself.
...
### Security
- **Device serials no longer leak inside the `scope_device_function` annotation.** Central returns `aruba-annotation:scope_device_function` as a *stringified* list of records, so the redaction walker saw one opaque string and never descended to the identifiers inside it. A scope of type `DEVICE` is named by the device's serial (verified live: 119/119 DEVICE scope nodes had `scope_name == serial`), so every `detailed=True` read shipped that serial in cleartext — while the *same* serial was tokenized under its own `serial` key elsewhere in the response, letting a reader undo the masking by correlation. The walker now parses the blob (JSON or Python-`repr`), tokenizes DEVICE `scope_name` as `SERIAL` (deduped to the same token as the `serial` key), and leaves GLOBAL/SITE/DEVICE_COLLECTION architecture labels cleartext per the existing privacy model.
- **Known cleartext embedded in a larger string is now re-masked.** The keymap replay only matched a value that was a known plaintext *in its entirety*, so a tokenized identifier survived cleartext the moment anything was concatenated around it — e.g. `management-users/<user>`. `Tokenizer.replace_known_cleartext` now substitutes known plaintexts embedded in a value, bounded by word edges and gated by a length floor so it cannot corrupt unrelated text.
- **Certificate-class fields no longer over-mask non-material scalars.** `TokenKind.CERT` fired on any value under a `cert`/`certificate`/`chain`/… key regardless of shape, so a certificate *name* — or a caller's own scalar that merely reused the word — was replaced with a `[[CERT:…]]` token. Because tokens are keyed by plaintext, identical throwaway values collided onto one token and read like a tokenizer fault. CERT now requires certificate material (PEM/OpenSSH armor, or a long opaque blob); all other exact-match secret kinds still fire unconditionally.

### Fixed
- **Code-mode `execute()` blocks no longer die on a bad tool argument.** FastMCP raises its own `fastmcp.exceptions.ValidationError` (a `FastMCPError`, **not** a subclass of pydantic's) for an unexpected/for-bad keyword argument during binding. `ValidationCatchMiddleware` caught only pydantic's type, so this escaped into the sandbox as a fatal error — a model probing a tool with a wrong kwarg lost its entire `execute()` block even inside `try/except`. The middleware now catches both, recovering the structured pydantic error from `__cause__` for a clean redacted summary (and stripping `input_value=` echoes when only the string form is available). Returns the normal `ok=False, status=422` envelope the model can branch on.
- **`central_get_wlan_profiles`** now accepts the eight config-read query parameters (`view_type` / `object_type` / `scope_id` / `device_function` / `effective` / `detailed` / `limit` / `offset`). It builds its own `api_path`, so the #624 sweep (which keyed on `_get_resource` callers) missed it; it now delegates to `_get_resource` and inherits the scope-guard and pagination handling.
- **`central_get_role_with_policy`** gains the six meaningful scope parameters, applied to **both** the role fetch and each bound-policy fetch so the bundle describes one consistent scope/view. `limit`/`offset` are intentionally omitted — it resolves named objects, not a collection.
- **`central_get_config_assignments`** gains the documented `profile_type` filter, and its `scope_id`/`device_function` parameters — which carried `Field(default=None)` but no Python default — can now actually be omitted, matching the docstring (a direct call previously raised `TypeError`).
...
- **9 more config-write tools resolve a payload schema.** The MRT sitemap/floor/building/webhook writes (`central_manage_building`, `_floor`, `_floor_walls`, `_floor_zones`, `_device_location`, `_sitemap_devices`, `_asset_tag_metadata`, `_notification_rule`, plus the `network-services` webhook) hit nested `network-monitoring/v1/...` paths that the flat network-config segment resolver couldn't map, so `get_schema` returned no body guidance and the model authored those bodies blind. `_central_segment` now falls back to the tail resource segment of any literal `api_path`, resolving them against the (already-vendored) spec. Body coverage for body-bearing writes is now 231/232 — the one remaining (`central_manage_cnac_job`) has no write body documented in the vendored spec.
...
- **Reactive error enrichment (part 3) — all non-2xx.** When a tool call fails with a non-2xx status from an upstream error payload, the response is enriched with the API's *documented* meaning of that code (`429` → "Too Many Requests — 5000 API-call/hour threshold", `403` → "Forbidden", …), and for input errors (`400`/`422`) on the opaque-body config/write tools, the legal body field set. Sourced from the spec index's `responses` table; status meanings are resolved per `(platform, code)` and used only when representative (EdgeConnect's per-endpoint descriptions are skipped rather than generalized). Hooked into `ResponseEnvelopeMiddleware` (upstream errors only — not our own gate/control blocked-states) and `ValidationCatchMiddleware`. This is the reactive half of the design — it catches the model that *guessed* and hit an error (the AP-rename failure loop), complementing the proactive `get_schema` path.
...
### Fixed
- **#591 (high) — transport failures no longer abort a run or corrupt resume state.** `_poll_async_operation` now contains HTTP/transport exceptions (transient-retry, then `None` → the batch is marked `timed_out`), so a 5xx/timeout after a 202-accepted POST/PATCH doesn't tear down the tool. This restores the `Never raises` contract of the assignment/enrichment PATCH helpers (their poll ran outside the protected block). The batch `POST` is wrapped too — a transport error records that batch as `timed_out` (server acceptance unknown, since a post-send timeout may have been accepted) and the run continues.
...
- **#589 — outbound tokenization now sweeps identifier-bearing dict *keys*.** Maps keyed by an email-shaped identifier or a MAC previously left the key cleartext (only values + wrapper keys were handled). Keys now get the same universal identifier tokenization + MAC normalization as values; inbound detokenization already restores tokenized keys, so the round-trip stays symmetric. (Bare, context-free strings like a username `alice` are left alone — same as values.)
...
### Added
- **EdgeConnect platform** (`edgeconnect_*`). 1216 tools across 187 tag-derived modules under `platforms/edgeconnect/tools/`, generated from `vendor/edgeconnect/EdgeConnect-9-7-REST-API.json` by a maintainer-local generator (generators are intentionally not committed — see `.gitignore`; the generated tool modules ARE committed and are the runtime source of truth). Tool names derive from HTTP method + URL path (`edgeconnect_<method>_<path-slug>`) because the spec's operationIds are unusable (687/1216 are auto-numbered, 12 collide); the 2 residual slug collisions are de-duped with a numeric suffix.
- **Dual-mode auth** (`platforms/edgeconnect/client.py`): API key (`X-Auth-Token` header) **or** username/password session login (`POST /authentication/login` → cookie jar + `orchCsrfToken` → `X-XSRF-TOKEN` header). Re-logs in once on 401; logs out on shutdown; injects `?source=menu_rest_apis_id` on every request; base path `/gms/rest`. No runtime dependency on the `pyedgeconnect` SDK (sync-only; SDK-removal policy #468/#470) — the documented raw-`requests` flow is ported to async httpx.
- **Capability classification** (conservative, method-based): `GET → READ` (657), `POST`/`PUT` → `WRITE` (470), `DELETE` → `WRITE_DELETE` (86), plus 3 side-effecting GETs (`/authentication/logout`, `/idle/clear`, `/idle/increment`) reclassified `OPERATIONAL` (visible but confirmation-gated, via a generator override). EdgeConnect uses POST for read-style "query with a body" calls, so those sit behind the write-gate too (the safe direction — nothing mutating is exposed un-gated). Writes enable with `ENABLE_EDGECONNECT_WRITE_TOOLS=true`, wired through **all** gate paths: the static `Visibility` transform, the meta-tool `is_tool_enabled` gate (`_GATE_CONFIG_ATTR`), and the elicitation write-enable middleware.
- **Request-body fidelity** in generated tools: required request bodies (`requestBody.required`) are emitted as required params (no `None` default); non-object bodies keep their real type (`list[...]`, `str`, …) instead of being forced to `dict[str, Any]`.
- **Content-type-aware transport**: the body's OpenAPI content type is carried through to `edgeconnect_request` (`body_mode`) and `EdgeConnectClient.request` so bodies are sent correctly — `application/json` via `json=`, `text/plain` via `content=`, `x-www-form-urlencoded` via `data=`; `multipart/form-data` uploads (5 ops) are explicitly unsupported and raise a clear 501 rather than being mis-sent as JSON.
- **Secrets**: `edgeconnect_host` (required) plus `edgeconnect_api_key` **or** `edgeconnect_user` + `edgeconnect_password`; optional `edgeconnect_login_type` (0=local/1=radius/2=tacacs) and `edgeconnect_verify_ssl`. Health probe via `GET /authentication/loginStatus`.

### Tests
- `test_edgeconnect_client.py` (auth modes, CSRF capture, source-param injection, 401 re-login) + `test_edgeconnect_tools.py` (1216-tool count, capability breakdown, name uniqueness, write-gate tags, config-loader matrix). The client test caught a real bug: `AsyncTokenManager` rejects an empty token, so the no-CSRF session path now returns a non-empty session sentinel (the cookie jar authenticates) instead of `""`.

### Docs
- README (platform list, comparison table, counts), `INSTRUCTIONS.md` (namespace + EdgeConnect section), `docs/TOOLS.md`, and the discovery-tool platform enumerations updated for the 9th platform.
...
### Added
- **Build-time sandbox prewarm** (`hpe_networking_mcp.prefab_prewarm`, run from the Dockerfile). The `generate_prefab_ui` renderer runs Prefab Python in a Deno + Pyodide subprocess; the first render per container otherwise pays a cold start that fetches `npm:pyodide` + `deno.land/std` and initializes the WASM runtime. The build now runs one throwaway render so the Deno module graph + Pyodide runtime are baked into the image's `DENO_DIR`. **Measured first-render cold start ~3.0s → ~1.5s**, and the first render no longer needs network egress (relevant for locked-down deployments). Best-effort by build network: builds **with** egress bake the cache and get the fast first render; builds **without** egress still produce a fully working image that simply falls back to the runtime cold-start (the prewarm logs and exits 0).
...
**Patch — refactor: capability classification (rubric + shared factory), Axis migrated (PR 1 of N).** First step of the tool-annotation/confirmation rework. Adds `platforms/_common/annotations.py` — a single `Capability` classification (`READ` / `DIAGNOSTIC` / `WRITE` / `WRITE_DELETE` / `OPERATIONAL`) from which `classify()` derives the MCP `ToolAnnotations` hints, the `<platform>_write[_delete]` enable tag, and a new `requires_confirmation` gate tag, so those can no longer drift apart per tool. The per-platform `@tool` shims are now built by one shared `make_tool_decorator` factory in `_common/tool_registry.py` (each platform's `_registry.py` shrinks to `tool = make_tool_decorator("<platform>", lambda: mcp)`); `ToolSpec` gains a `capability` facet for discovery/search. `_template` (the canonical reference) and **Axis** are migrated to `@tool(capability=...)`: `axis_get_*` → READ, `axis_manage_*` → WRITE_DELETE, and `axis_commit_changes` / `axis_regenerate_connector` → OPERATIONAL (`enable_gated=True`, preserving their `axis_write_delete` gating). Behaviour-preserving: the `requires_confirmation` tag is inert until the future universal-gate PR; existing per-tool elicitation and enable-gating are unchanged. Rubric documented in `docs/tool-annotation-rubric.md`. Other platforms migrate in subsequent PRs; no release ships in a half-migrated state. Covered by `tests/unit/test_capability_annotations.py` plus updated Axis/template tests.

## [3.2.3.10] - 2026-06-03

**Patch — fix: code-mode write confirmation no longer silently self-declines.** In code mode (`MCP_TOOL_MODE=code`), destructive writes (create / update / delete) dispatched through `<platform>_invoke_tool` ran in a nested `call_tool` context that does **not** inherit the `initialize` request's `elicitation_mode` state. With the mode resolving to `None`, the tool never surfaced a confirmation prompt yet still returned `"Action declined by user."` — a decline the user never made — and the AI, taking the fabricated veto at face value, stalled. New `_resolve_elicitation_mode()` in `middleware/elicitation.py` re-derives the mode from `config` (reachable via `lifespan_context` in every context) when request state is absent and defaults to `chat_confirm`, so the tool returns `confirmation_required` and the AI confirms in chat then re-calls with `confirmed=true`. A genuinely surfaced-and-declined prompt still returns a real decline; `DISABLE_ELICITATION=true` still auto-accepts (previously it too was broken in code mode, blocking writes entirely). Single-point fix covering both `confirm_write` callers and the inline `central_manage_*` guards. Covered by `tests/unit/test_elicitation_mode_resolution.py` (13 cases incl. the exact reported path). Closes #413.
...
Reproduced live on a 6100 access switch (firmware PL.10.16.1020) with config-health as the verification primitive. Five error/response patterns captured verbatim from the real API:

1. **`HTTP 400 "Cannot find in library 'sys_policy_pap_X' of type 'aruba-policy' referred in '<vlan-id>' of type 'aruba-interface-vlan'"`** — binding a device-rendered name to an SVI at any scope where the name isn't in the policy library. Library-membership check; identical format for any unknown name (sys-prefix is not special).
2. **`HTTP 400 "Validation failure: Role-based Policies cannot be mapped to ScopeTypes.DEVICE."`** (and `.DEVICE_COLLECTION`) — direct scope-assignment of an `ASSOCIATION_ROLE` policy at narrow scope. Central enforces this at the scope-write layer.
3. **`HTTP 500 "Cannot find object 'X' of module 'aruba-policy'"`** — creating the SVI at a DEVICE or DEVICE_COLLECTION scope with a policy reference, where the policy isn't *directly assigned* at that scope (effective-config inheritance doesn't satisfy this validator).
4. **`HTTP 400 "Validation failure: Policies {'X'} still part of Policy Group."`** — trying to delete a policy still listed in the singleton policy-group's `policy-group-list`. The CNX UI adds the entry implicitly when the operator creates a role-based policy; the skill's per-tool path requires `central_manage_policy_group_entry(action_type='update', ...)` before scope-assignment and reverse-order cleanup.
5. **The silent-skip-with-success-indication trap** — the worst failure mode in the catalogue. Bind the role-based policy by its CNX library name at SITE scope: Central accepts with HTTP 200, lists it in the device-scope effective config, pushes the layer2 VLAN to the device, **silently skips the role-based policy render** (no `sys_policy_pap_<role>` appears on the device), and reports the device as `SYNCHRONIZED` with `activeIssues: []` and `topPriorityIssue: "-"`. No error anywhere. The operator has no signal that anything failed. **Earlier guesses (this one included) had assumed OUT_OF_SYNC; live test proved the actual mode is silent-skip — strictly worse than OUT_OF_SYNC because there is no signal to investigate.**

Skill changes:
- **Frontmatter triggers extended** with the wrong-primitive symptom phrases (`sys_policy_pap_*`, `sys_pap_*`, `port-access role`, `role-based policy won't apply to VLAN`, `Cannot find in library 'sys_policy_pap_…'`, `Role-based Policies cannot be mapped to ScopeTypes.DEVICE`, `the CNX UI won't let me apply the policy to a VLAN`) AND the direct CLI-paste apply patterns (`apply policy sys_policy_pap_`, `apply policy <NAME> routed-in`, `interface vlan <N> apply policy`, `MCP, do this: interface vlan … apply policy …`) so an operator pasting that exact wording into a fresh AI session matches the discovery layer.
- **Tools list extended** with the diagnosis / verification surface: `central_get_role_with_policy`, `central_manage_role`, `central_get_policy_groups`, `central_manage_policy_group_entry`, `central_get_config_assignments`, `central_manage_config_assignment`, `central_get_effective_config`, `central_get_devices_config_health`, `central_show_commands`.
- **The three rules grew to five.** Rule 4: POLICY_QOS has two associations; pick `ASSOCIATION_INTERFACE` for SVI bind. Rule 5: `SYNCHRONIZED` is not proof of correctness — every device-touching push must end with a `central_show_commands` round-trip that confirms the artifacts actually rendered on the box.
- **New section "Path picker — POLICY_QOS comes in two flavors"** before the existing A/B/C shapes. Comparison table (INTERFACE vs ROLE: device-side render, enforcement domain, SVI-bindable, intended use), the five-row error → cause table, and a **paste-ready wrong-primitive runbook code block** that strips the `sys_policy_pap_` / `sys_pap_` prefix, reads the policy via `central_get_policies`, checks the `association` field, and either pivots to A/B/C (with a remediation paragraph) or proceeds with SVI bind. Also documents the `policy-group-list` membership step with the `action_type='update'` quirk (the per-entry tool rejects `'create'` because the singleton container exists).
- **Section B (marker policy)** gets an explicit field note: `association` MUST be `"ASSOCIATION_INTERFACE"` for SVI bind; pivot if the source was `ASSOCIATION_ROLE`.
- **Section C (SVI bind)** gets the error → cause table for the bind step (the three error patterns from the same row in the Path picker), plus an explicit "verify on the device via `central_show_commands`" step — `config-health` alone is documented as insufficient.
- **Steps section** adds a new **Step 0 — Wrong-primitive check** that runs the runbook when any symptom matches, and reinforces the device-side `show` verification at step 5.

Skill grew 543 → 731 lines; still in range (`aos-migration.md` is 1598). Skill-only change; no tool-count change.

## [3.2.3.3] - 2026-05-28

**Patch — extend the `central-qos-policy` skill with the egress / system-wide half (queue-profile, schedule-profile, qos-global apply + DSCP-map, update-applied-profile ritual).** The v3.2.3.1 hardening covered the ingress / marking pipeline (traffic class → marker policy → SVI bind) but stopped before the system-wide queueing tier — operators hit the wall the moment a CLI config carried `qos queue-profile` / `qos schedule-profile` / `apply qos queue-profile … schedule-profile …` / `qos dscp-map …` lines. Added four new sections to the skill with live-round-trip-verified payload shapes:

- **D. Queue-profile (`central_manage_qos_queue`)** — local-priority → hardware queue mapping. `priority[]` array, one entry per queue, each entry's `priorities` is a list (a queue can absorb multiple LPs). Client-side validation rules carried in the skill: all 8 LPs mapped exactly once, no duplicates. Calls out the `payload_schema` distiller gap: top-level `priority` is annotated PVOS-only but the API accepts the shape on CX (verified).
- **E. Schedule-profile (`central_manage_qos_schedule`)** — per-queue scheduling. `sched-entries[]` array. **The `WEIGHTED` algorithm enum collapses both DWRR and WFQ** — there is no CLI-style split at the API; `weight: 1` per queue matches factory-default WFQ behavior. `STRICT` with optional `max-bandwidth-kbps` XOR `max-bandwidth-percent`; `MIN_BANDWIDTH` with `minimum-bandwidth` percent. Single-algorithm-with-strict-tail rule (only the highest-numbered queue may be STRICT) carried as a client-side validation rule.
- **F. System-wide apply + DSCP-map (`central_manage_qos_global`)** — one named library object holds both `q-profile` and `sched-profile` refs (each a `oneOf`: custom profile name string OR the literal `"DEFAULT"`) plus the `dscp-map[]` sub-array. The CLI atomic `apply qos queue-profile X schedule-profile Y` and the `qos dscp-map …` block collapse to **one write**, then a scope assignment to activate on devices. Five translation points called out for small models: DWRR/WFQ collapse, `"DEFAULT"` ≠ `"factory-default"`, qos-global is a named object not a singleton, dscp-map lives inside qos-global, and **PATCH semantics — field-preserve + array-upsert-by-key** (top-level fields you don't send are preserved; array entries are merged by `x-key`, never wholesale-replaced — verified live).
- **G. Update-applied-profile automated ritual** — the qos-global API rejects in-place edits to a queue- or schedule-profile while it's currently applied; the spec documents a flip → edit → restore recovery procedure. Packaged as one paste-ready code block (inputs at top) that: reads current qos-global, detects whether the ritual is needed (skips entirely if the profile isn't currently applied), flips qos-global to `DEFAULT`/`DEFAULT` with read-back assertion, edits the profile, **always restores the original refs even on edit failure** (rollback path), and prints a structured `status` dict with `steps`/`failed_at`/`final_state` for operator-facing reply. The PATCH semantics finding means rollback state is just two strings (`old_q` + `old_s`), and the `dscp-map` survives the flip intact.

Skill grew 230 → 543 lines; well within the established range for unified runbooks (`aos-migration.md` is 1598). Skill-only change; no tool-count change; the four `central_manage_qos_*` tools were already registered. Frontmatter `tools:` list extended to advertise the six new (get + manage × 3) tools the skill now uses.
