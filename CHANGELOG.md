...
- **Capability classification** (conservative, method-based): `GET → READ` (660), `POST`/`PUT` → `WRITE` (470), `DELETE` → `WRITE_DELETE` (86). EdgeConnect uses POST for read-style "query with a body" calls, so those sit behind the write-gate too (the safe direction — nothing mutating is exposed un-gated). Writes enable with `ENABLE_EDGECONNECT_WRITE_TOOLS=true`.
