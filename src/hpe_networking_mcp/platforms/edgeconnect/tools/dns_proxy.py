...
@tool(
    name="edgeconnect_get_dns_proxy_config",
    description="GET /dnsProxy/config\n\ngetDnsProxyConfig\n\nGet DNS proxy configuration for an appliance",
    capability=Capability.READ,
)
async def edgeconnect_get_dns_proxy_config(
    ctx: Context,
    nePk: Annotated[
        str,
        Field(
    ...
    cached: Annotated[
        ...
        Field(
            default=None,
            description="Controls data retrieval source. When 'true' (default), retrieves data from Orchestrator cache. When 'false', fetches directly from the appliance and updates the cache.",
        ),
    ] = None,
) -> Any:
