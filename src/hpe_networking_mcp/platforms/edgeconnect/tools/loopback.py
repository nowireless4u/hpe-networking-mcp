...
@tool(
    name="edgeconnect_get_virtualif_loopback",
    description="GET /virtualif/loopback\n\nloopbackGet916\n\nGet loopback interface configuration",
    capability=Capability.READ,
)
async def edgeconnect_get_virtualif_loopback(
    ctx: Context,
    nePk: Annotated[
        str,
        Field(
    ...
    cached: Annotated[
        ...
        Field(
            default=None,
            description="Data retrieval source. When 'true' (default), returns cached configuration from Orchestrator database. When 'false', fetches real-time configuration directly from the appliance and updates the cache.",
        ),
    ] = None,
) -> Any:
