...
@_mcp_tool(
    ...
    description="GET /api/v1/sites/{site_id}/stats/bgp_peers/count\n\ncountSiteBgpStats\n\nCount BGP peer statistics for a site, optionally grouped by the `distinct` field and filtered by peer state. Use [Count Org BGP Stats](/#operations/countOrgBgpStats) to count BGP peer statistics across the organization.",
    capability=Capability.READ,
)
async def mist_count_site_bgp_stats(
    ctx: Context,
...
@_mcp_tool(
    ...
    description="GET /api/v1/sites/{site_id}/stats/bgp_peers/search\n\nsearchSiteBgpStats\n\nSearch BGP peer statistics for a site with filters for device, neighbor, VRF, and time range. Use [Search Org BGP Stats](/#operations/searchOrgBgpStats) to search BGP peer statistics across the organization.",
    capability=Capability.READ,
)
async def mist_search_site_bgp_stats(
    ctx: Context,
    site_id: Annotated[str, Field(description="path parameter 'site_id'")],
