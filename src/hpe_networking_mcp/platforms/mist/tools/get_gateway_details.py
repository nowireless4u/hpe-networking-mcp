"""Mist tool: get detailed information for a specific gateway."""
...
@tool(
    ...
    description=(
        ...
        "model, firmware, interfaces, tunnels, IP, and status."
    ),
    tags={"devices"},
    annotations={
        "title": "Get gateway details",
        "readOnlyHint": True,
...
async def get_gateway_details(
    ...
    device_id: Annotated[
        UUID,
        Field(
            description=("Gateway device ID (use mist_search_device to find it)"),
        ),
    ],
) -> dict | list | str:
    """Get detailed information for a specific gateway."""

    logger.debug("Tool get_gateway_details called")
    logger.debug(
