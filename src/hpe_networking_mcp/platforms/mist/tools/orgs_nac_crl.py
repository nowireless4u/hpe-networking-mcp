...
async def mist_delete_org_nac_crl(
    ...
    naccrl_id: Annotated[str, Field(description="path parameter 'naccrl_id'")],
) -> Any:
    return await mist_request(
        ctx,
        "DELETE",
        "/api/v1/orgs/{org_id}/setting/mist_nac_crls/{naccrl_id}",
        path_params={"org_id": org_id, "naccrl_id": naccrl_id},
        query_params=None,
        body=None,
    )
...
async def mist_import_org_nac_crl(
    ...
    body: Annotated[
        dict[str, Any] | None,
        Field(default=None, description="Request body for POST /api/v1/orgs/{org_id}/setting/mist_nac_crls"),
    ] = None,
) -> Any:
    return await mist_request(
        ctx,
