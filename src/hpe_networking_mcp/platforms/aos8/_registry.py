"""AOS8 tool registry -- module-level FastMCP holder + shared-registry shim.

Tool modules under ``platforms/aos8/tools/`` decorate their functions with
...
3. Populates ``REGISTRIES["aos8"]`` so the three AOS8 meta-tools can
...
# Set by platforms.aos8.register_tools() before tool modules are imported.
...
def _derive_category(func: Callable[..., Any]) -> str:
    """Short module name as the registry category (e.g. ``health``)."""
