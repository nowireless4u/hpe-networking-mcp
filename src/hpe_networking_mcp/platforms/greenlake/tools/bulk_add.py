...
async def greenlake_bulk_add_devices(
    ...
    if not result.valid_rows:
        if result.invalid_rows:
            ...
            raise ToolError({"status_code": 400, "message": f"no valid rows found in CSV. Validation failures: {details}"})  # noqa: E501
        raise ToolError({"status_code": 400, "message": "no valid rows found in CSV"})

    # SECTION 1 — Write gate: require human confirmation before touching production
    decision = await confirm_write(
        ctx,
        message=(
            f"Bulk-add {len(result.valid_rows)} device(s) to HPE GreenLake. "
            "Serial numbers and MAC addresses will be transmitted to the API. "
            "Confirm this complies with your organization's data-handling policy."
        ),
    )
    if decision is not None:
        return decision

    # SECTIONS 2-3 — Cache path + resume-skip setup
