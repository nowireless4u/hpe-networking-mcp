...
async def _patch_assign_application(
    ...
    try:
        response = await client.patch_raw(endpoint, data=body, additional_headers=headers, params={"id": device_uuid})
    ...
    # CONFIRMED (2026-06-28): PATCH /devices/v2beta1/devices?id=<uuid> returns 202
    # with a Location header whose final segment is the async-operation UUID under
    # /devices/v1/async-operations/.  Both application.id AND region are required —
    # omitting region returns 400 HPE_GL_DM_BAD_REQUEST.
...
def setup_resume(
    ...
    if csv_path:
        ...
        digest = _hashlib.sha256(csv_text.encode()).hexdigest()[:16]
        cache_path = pathlib.Path(tempfile.gettempdir()) / f"greenlake_bulk_{digest}.cache.json"

    existing_cache = _load_cache(cache_path)
    # Filter cache to only serials present in the current CSV.  If the operator
    # edits the CSV between runs (removes rows, retries with a smaller file, etc.)
    # stale entries would otherwise inflate succeeded/failed counts and prevent
    # cache deletion even after all current-run devices finish.
    current_serials = {row["serialNumber"] for row in valid_rows}
    filtered_cache = {sn: v for sn, v in existing_cache.items() if sn in current_serials}
    pending_rows = [
        {**row, "_row_index": idx}
        for idx, row in enumerate(valid_rows)
        if filtered_cache.get(row["serialNumber"], {}).get("status") != "succeeded"
    ]
    skipped_count = len(valid_rows) - len(pending_rows)
    return cache_path, pending_rows, skipped_count, filtered_cache
