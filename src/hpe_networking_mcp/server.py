...
_GENERATIVE_UI_GUIDANCE = (
    ...
    "first (platform read tools / an `execute` block), then INLINE it into the `code` "
    "you pass here and compose the view from Prefab components (metric cards, "
    "bar/line/area/pie charts, data tables, badges) with reactive state for in-window "
    "filtering. Do NOT hand-write raw HTML as a text response for visualization "
    "requests — render it through this tool so the client shows a live, interactive "
    "widget instead of a static blob.\n\n"
    "DATA CONTRACT (avoids the #1 error — read carefully): your `code` must be "
    "SELF-CONTAINED. INLINE the values you gathered directly into the `code` as Python "
    "literals (e.g. start the code with `devices = [{...}, {...}]` / "
    "`site = {...}`), then reference those names. Do NOT pass a `data` argument — the "
    "widget renderer executes your `code` in the browser AS IT STREAMS and does NOT "
    "receive the `data` argument's globals there, so any name that came only from "
    "`data` raises `NameError: name 'X' is not defined` and the widget hangs forever on "
    "'waiting for content'. Everything the `code` references must be defined within the "
    "`code` itself. Embed the real values you collected (do not invent placeholders).\n\n"
    "DATA SHAPE (unwrap before inlining): platform reads return an envelope — "
    ...
    "further `result` / `items`), then inline those unwrapped values — never inline the "
    "raw envelope or you render bookkeeping keys instead of your values.\n\n"
...
def _register_code_mode(mcp: FastMCP, max_duration_secs: float = 30.0) -> None:
    ...
    execute_description = (
        ...
        "you looked up dynamically. Either way, write/destructive tools fire the "
        "same confirmation prompt before running — the gate is structural at the "
        "tools/call layer, so you cannot skip it by calling a tool directly.\n"
