...
from hpe_networking_mcp.translations.readers.aos8._common import index, leaf, members, opmode_key
...
def _rad_server(rec: dict, port_subkey: str, default_port: int) -> RadiusServer:
    host = leaf(rec.get("rad_host"), "host") or ""
    port = leaf(rec.get(f"rad_{port_subkey}"), port_subkey) or default_port
    secret = leaf(rec.get("rad_key"), "key")
...
def _radius(
    ...
    if not aaa_prof:
        return None
    dot1x_sg = leaf(aaa_prof.get("dot1x_server_group"), "srv-group")
    mba_sg = leaf(aaa_prof.get("mba_server_group"), "srv-group")
    acct_sg = leaf(aaa_prof.get("rad_acct_sg"), "server_group_name")
    coa_ips = [
        c["rfc3576_server"]
    ...
    def servers_of(sg_name: str | None, port_subkey: str, default_port: int) -> list[RadiusServer]:
        ...
        for m in members(server_groups.get(sg_name or "")):
...
def _vlan(virtual_ap: dict) -> Vlan:
    val = leaf(virtual_ap.get("vlan"), "vlan")
    if val is None:
...
def _security(ssid_prof: dict, aaa_prof: dict | None, server_groups, auth_servers) -> Security:
    triplet = _OPMODE_TO_TRIPLET.get(opmode_key(ssid_prof.get("opmode")) or "")
    if triplet:
    ...
    mba = leaf((aaa_prof or {}).get("mba_server_group"), "srv-group")
    sec.mac_auth = bool(mba)

    if km in (KeyMgmt.PSK, KeyMgmt.SAE):
        psk = leaf(ssid_prof.get("wpa_passphrase"), "wpa-passphrase")
        if psk:
            sec.psk = psk
...
def aos8_read_wlan(
    ...
    *,
    ...
    """
    sp_by = index(ssid_profiles, "profile-name")
    aaa_by = index(aaa_profiles, "profile-name")
    # AOS 8 server-groups (server_group_prof) key on ``sg_name``, not profile-name.
    sg_by = index(server_groups, "sg_name", "profile-name")
    as_by = index(auth_servers, "rad_server_name", "tacacs_server_name")

    sp_name = leaf(virtual_ap.get("ssid_prof"), "profile-name")
    ssid_prof = sp_by.get(sp_name or "", {})
    aaa_name = leaf(virtual_ap.get("aaa_prof"), "profile-name")
    aaa_prof = aaa_by.get(aaa_name or "")

    essid = leaf(ssid_prof.get("essid"), "essid") or virtual_ap.get("profile-name", "")
    ...
    return CanonicalWlan(
        ...
        enabled=True,
        hidden=bool(leaf(ssid_prof.get("hide_ssid"), "hide-ssid")),
