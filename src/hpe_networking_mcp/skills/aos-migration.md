...
2. After the operator uploads, the file is held in the server's session store. Call `list_files` to get the uploaded file's **name** (metadata only — name/size/type, no content), then parse it **server-side**: `aos8_parse_config(filename="<uploaded filename>")`. The tool reads the file *inside the server* so the raw config — including PSKs and RADIUS/TACACS secrets — never enters the model context. There is intentionally **no tool that returns the raw uploaded file to you**; do not attempt to read the file contents directly.
...
| A2 | **GreenLake AOS subscription capacity** ≥ source AP count | `greenlake_get_workspaces_v1_workspaces_workspace_id()` + `greenlake_get_subscriptions_v1_subscriptions()` | always | REGRESSION if AP count > active AP-license count |
