...
**Optional MCP-Apps providers** (`MCP_APP_ENABLE=true`, off by default — one switch for both): `file_manager` (drag/pick upload UI) plus `list_files` (file metadata only — name/size/type), and `generate_prefab_ui` (the model writes a live Prefab dashboard from data it collected) plus `search_prefab_components`, are exposed at the top level — even in code mode. The UIs render only in MCP-Apps hosts (Claude Desktop / ChatGPT / claude.ai); elsewhere these are callable tools with no rendered widget. Use `file_manager` to let an operator upload a file (e.g. an AOS 8 config for `aos-migration`, or a device CSV for `greenlake_bulk_add_devices`) instead of pasting it; use `generate_prefab_ui` for any dashboard / visualization request.

**Uploaded files are read SERVER-SIDE only.** There is deliberately **no tool that returns raw uploaded file content to the model** — the `read_file` tool is removed precisely so sensitive upload contents (device serials/MACs, AOS 8 configs carrying PSKs and RADIUS/TACACS secrets) never enter the model context. Discover an uploaded file's name with `list_files` (metadata only), then hand that name to the consuming tool, which reads it inside the server: `greenlake_bulk_add_devices(csv_filename="…")`, `aos8_parse_config(filename="…")`. Do not try to read upload contents directly.
...
* **In-sandbox discovery** (works regardless of what the client loaded): from inside `execute()`, call `await call_tool("<platform>_list_tools", {"filter": "..."})` to get a name+params catalog, then `await call_tool("<platform>_get_tool_schema", {"name": "..."})` for full schemas, then dispatch via `await call_tool("<platform>_invoke_tool", {"name": "<tool_name>", "params": {...}})`. Useful as a fallback when the client's semantic tool_search didn't surface `search` / `tags` / `get_schema` (verified to happen on Claude Desktop / CoWork for queries like "list mist sites" — see issue #302).
  ...
  **Per-platform tools are callable two equivalent ways** — directly by their bare name (`await call_tool("mist_search_org_devices", {...})`) or via `<platform>_invoke_tool` (`await call_tool("mist_invoke_tool", {"name": "mist_search_org_devices", "params": {...}})`). Both resolve for every platform, Mist included (the ~1000 spec-driven Mist tools register with FastMCP just like Central's — fixed in v3.4.5.6 / #524; before that they were registry-only and a direct call raised `Unknown tool`). `invoke_tool` is convenient when dispatching a name you looked up dynamically.
...
> **Code mode caveat:** of the cross-platform tools above, `health`, `translate_wlan_preview` / `translate_wlan_apply`, and `translate_config_preview` / `translate_config_apply` are registered in `code` mode (reachable from `execute()` via `call_tool`); `site_health_check` and `site_rf_check` are deliberately NOT (code mode's premise is that the AI composes per-platform tools itself). In code mode:
...
| *"UCC call quality"*, *"check call quality on &lt;AP/site&gt;"*, *"voice quality"*, *"is my WiFi Calling / Zoom / Teams call OK"*, *"VoWiFi quality"*, *"UCC dashboard"*, *"show me active calls on &lt;AP&gt;"*, *"why is my call choppy / laggy"*, *"UCC score"*, *"is voice degraded"*, *"show ucm cdrs / hashtable"*, *"datapath session ucc"* (Aruba Central AOS-10 APs / gateways) | `central-ucc-quality` |
| *"dashboard for site &lt;name&gt;"*, *"show me a dashboard for &lt;site&gt;"*, *"render a status board / scorecard for &lt;site&gt;"*, *"site overview for &lt;site&gt;"*, *"visualize site health for &lt;site&gt;"*, *"draw a dashboard of &lt;site&gt;'s devices and alerts"*, *"show NAC / who's connected at &lt;site&gt;"* (single Aruba Central site — health, devices by type, clients, alerts + a ClearPass NAC overlay of active sessions on the site's APs, via Generative UI) | `central-site-dashboard` |
| *"add devices to GreenLake"*, *"onboard devices"*, *"register / import devices"*, *"add APs / switches / gateways to GreenLake"*, *"bulk add devices"*, *"subscribe my new devices"*, *"add devices and assign subscriptions"* — the full add → subscribe → assign-service lifecycle, with the choose-upload-vs-paste step and the mandatory paste-visibility warning. Do NOT call `greenlake_bulk_add_devices` bare. | `greenlake-device-onboarding` |
...
- **Central collection reads put the rows under `result["data"]["items"]` — not a bare list.** As of v3.4.x (#491) the hand-curated Central reads expose a uniform collection envelope: `result["data"]` is a **dict** `{"items": [...]}` (plus optional `total` / `next` siblings), and the rows live at `result["data"]["items"]`. This covers `central_get_aps`, `central_get_ap_wlans`, `central_get_clients`, `central_get_devices`, `central_get_wlans`, `central_get_site_health`, and `central_get_site_name_id_mapping` (now a list of site objects under `items`, each with a `site_name` field — no longer a dict keyed by name). The `mrt_*` monitoring family (`network-monitoring/v1/*` — e.g. `central_get_switches`, `central_get_gateways`, the `*_radios` / `*_ports` reads) passes the raw API envelope through, which is also `{"items": [...], "next": ..., "total": ...}`. So the rule is uniform: **read `data["items"]`.** (Spec-driven `mist_*` / `edgeconnect_*` reads mirror their upstream API shapes and are not covered by this contract.)
...
- **Aliases, Server Groups, Named VLANs**: central_get_aliases, central_get_server_groups, central_get_named_vlan
  ...
  - Use `central_get_named_vlan` to resolve a named VLAN (from vlan-name) to its actual VLAN ID. If the VLAN ID uses an alias, resolve via `central_get_aliases`
...
- **Gateway Cluster Intent (GCIS)**: central_get_gw_cluster_intent_config, central_manage_gw_cluster_intent_config
  ...
  - **Key field — `cluster-mode`**: `CM_SITE` (auto-cluster at Site level — Central creates `auto_*` realized profiles automatically) or `CM_MANUAL` (auto-formation disabled — operator creates realized profiles via `central_manage_gateway_clusters`).
...
- **Gateway Clusters (realized)**: central_get_gateway_clusters, central_manage_gateway_clusters
...
- **MRT — Monitoring expansion (v3.1.2.0+)**: complete `network-monitoring/v1` coverage beyond the existing AP / client / device / gateway / switch / WLAN / site-health basics.
  ...
  - **Topology** (`mrt_topology.py`): `central_get_topology`, `central_get_neighbours`, `central_get_unmanaged_device`, `central_get_isolated_devices`, `central_get_device_inventory`, plus device writes `central_update_device_notes` (PATCH — updates the device-overview notes field only; use `central_manage_system_info` `hostname` to rename an AP) and `central_delete_device`.
...
- **MRT — Reporting (v3.1.2.0+)** (`mrt_reporting.py`): `central_get_reports`, `central_get_report_runs`, `central_get_reports_metadata` (available report types + KPI widgets), `central_get_report` (single definition), `central_update_report` (PUT — wholesale replace), `central_create_report` (write — new report definition).
- **MRT — Application visibility (v3.5.3.0+)** (`mrt_applications.py`): `central_get_application_categories`, `central_get_website_categories` (top categories by usage over a `start_at`/`end_at` window), `central_get_security_risks` (per-application risk scoring). All take the shared usage-scope filters (site/client/serial/ssid/user-role) and return the `{items, count, total, offset}` shape.
- **MRT — Notification rules (v3.5.3.0+)** (`mrt_notifications.py`): `central_get_notification_rules` / `central_get_notification_rule` reads; `central_manage_notification_rule` for **create + delete** (a rule needs an enabled `source.alerts` and an enabled `destination`). Update is intentionally unsupported — the notification-rules PATCH endpoint is gateway-blocked through the current client (issue #606); recreate the rule to change it. Distinct from webhooks (`mrt_webhooks`).
...
- **Config translation (AOS 8 → Central)**: `translate_config_preview` / `translate_config_apply` (**registered in every mode, incl. code** — reachable from `execute()` via `call_tool`)
  - Bridge to the canonical translation engine for the 12 non-WLAN config kinds: `vlan_id`, `named_vlan`, `net_group`, `role`, `policy`, the AAA chain (`auth_server`, `server_group`, `dot1x_auth`, `mac_auth`, `captive_portal`, `aaa_profile`), and `gateway_cluster`. Per-record: one source object per call.
  - `translate_config_preview(source_platform="aos8", target_platform="central", kind, source_record, scope_id, device_functions?, extra_ctx?)` — read-only; returns `{canonical, calls, unresolved, preview}` with secrets redacted. Use for "show me what migrating these AOS 8 policies / roles / VLANs would produce in Central". The `aos-migration` skill's Stage 9b drives it.
  - `translate_config_apply(...)` — executes against Central; gated by `ENABLE_CENTRAL_WRITE_TOOLS` + confirmation; idempotent (ensure-or-create). **Blocks `kind="auth_server"` with a 403** (cleartext shared secret) until secret tokenization ships — preview it and create auth-servers manually.
  - Kind-specific `extra_ctx`: `role_records` (full AOS 8 role list) for `policy` so the reader computes role attribution per ACL; `coa_servers` (the `aaa_prof.rfc3576_client[]` entries) for `auth_server` so co-located RFC 3576 / CoA servers fold onto the matching RADIUS server (#322); `cluster_strategy` (`ha_only` / `intent_site` / `intent_manual`) for `gateway_cluster`; `alias_name` to override the named_vlan alias.
  - **AAA chain** (Gateway-targeted to `MOBILITY_GW`): translate in dependency order — auth-server / server-group / captive-portal / dot1x-auth / mac-auth first, then the keystone aaa-profile (references the others by name), then the consuming wlan/role objects.
  - **Gateway clustering:** `central:gateway_cluster` (AOS 8 `cluster_prof` → Central dual-object clustering) gates every tunneled SSID — translate it before the tunnel-mode `wlan_ssid`. Its `cluster_strategy` runtime value forks the emit: `ha_only` (HA `gateway-clusters` profile only), `intent_site` (+ `gw-cluster-intent-config` `CM_SITE`), `intent_manual` (+ `CM_MANUAL`). Created LOCAL at the supplied `central_scope_id` (CM_SITE → global/site/site-collection; CM_MANUAL → device-group).
  - **WLAN SSID:** `central:wlan_ssid` (AOS 8 `virtual_ap` + joined `ssid_prof` → Central `wlan-ssids` + conditional `overlay-wlan` cluster binding). Runtime values: `target_mode` (the operator's per-SSID forward-mode decision — `bridged` / `tunneled` / `hybrid` / `bridged_and_tunneled`), `ssid_profiles` (the VAP's-scope `ssid_prof` list, joined by name), `gw_cluster_list` (the clusters a tunneled/hybrid/dual SSID binds to — **REQUIRED + non-empty for every overlay mode** (tunneled/hybrid/dual); the preprocessing raises if missing, since a gateway-terminated SSID with no cluster is invalid — issue #438), and — for `bridged_and_tunneled` only — `bridge_scope_id` + `tunnel_scope_id` (the per-location split). Maps essid, `opmode` (AOS 8 → Central enum), forward-mode, and VLAN (`NAMED_VLAN` → `central:named_vlan`). `bridged_and_tunneled` emits an ESSID alias + a bridge profile + a tunnel profile (both sharing the alias) + the tunnel overlay — the same SSID in both modes across locations. Deferred: inline bridge-mode AAA, radio/QoS, and the PSK secret (PII-gated).
  ...
  - Also returns **`target_collisions`** — distinct source records that resolve to the **same Central object** (e.g. case-folded named-VLAN aliases `USER-VLAN` + `user-vlan` → one `user-vlan` alias) and would overwrite/409 each other on execution (issue #439). Keyed by target-object identity (path-addressed resources by URL; `/config-assignments` by `profile-type`+`profile-instance`, so distinct assignments don't false-positive). Empty when none; surface non-empty collisions to the operator verbatim — they're write-time hazards, not skips.
...
- **Config-Model (v3.1.1.0+ bulk import — 389 tools across 19 modules)**: covers the rest of the Aruba Central configurable surface that wasn't already hand-curated. Every device-level config object reachable via `/network-config/v1alpha1/<segment>` now has a `central_get_<type>` and (where the spec supports it) a `central_manage_<type>` tool. Coverage areas:
  - **AAA & Auth**: `central_get_aaa_profile`, `central_get_dot1xauth`, `central_get_dot1xsupp`, `central_get_macauth`, `central_get_captive_portal`, `central_get_stateful_dot1x_profiles`, `central_get_auth_servers`, `central_get_auth_server_global_config`, `central_get_auth_survivability` + manage pairs. Use for designing AAA flows.
  - **Central NAC (CDA)**: `central_get_auth_profiles`, `central_get_authz_policies`, `central_get_identity_stores`, the five CDA portal types (`central_get_portals`, `central_get_overrides`, `central_get_skins`, `central_get_custom_messages`, `central_get_default_custom_messages`), `central_get_static_tag`, etc. Central NAC is a real cloud-native NAC product — these tools manage it directly. `central_get_cnac_job_status` polls a CDA import/export job by id.
  - **Certificates & PKI**: `central_get_certificates`, `central_get_certificate_store`, `central_get_certificate_rcp`, `central_get_device_certificates`, `central_get_certificate_usage`, `central_get_gw_certificate_usage`, `central_get_est_profiles` + manage pairs. **Sensitive payloads** — see PII caveat below.
  - **Routing & Overlays**: `central_get_bgp`, `central_get_ospfv2` / `_ospfv3`, `central_get_rip`, `central_get_pim_router`, `central_get_evpn`, `central_get_vrfs`, `central_get_static_route`, `central_get_route_maps`, `central_get_bfd`, etc.
  - **Interfaces**: `central_get_ethernet_interfaces`, `_loopback`, `_vlan`, `_management`, `_portchannel`, `_profile`, `_subinterface`; `central_get_ap_port_profiles`, `central_get_gw_port_profiles`, `central_get_sw_port_profiles`; `central_get_lldp`, `central_get_cdp`, `central_get_lacp`, `central_get_sflow`, etc.
  - **Network Services**: `central_get_dhcp_pool` / `_relay` / `_server` / `_snooping`, `central_get_qos_global` / `_queue` / `_schedule`, `central_get_mgmd_global`, `central_get_ipsla`, `central_get_ip_source_lockdown`, etc.
  - **VLANs & L2**: `central_get_layer2_vlan`, `central_get_vrrp`, `central_get_stp`, `central_get_erps`, `central_get_loop_protect`, `central_get_mvrp`, etc.
  - **System**: `central_get_system_info`, `central_get_snmp`, `central_get_ntp`, `central_get_dns`, `central_get_logging`, `central_get_management_users`, `central_get_packet_capture`, etc.
