...
- **Common kinds:** `PSK` (WPA/SAE keys, passphrases), `RADSEC` (RADIUS shared secrets, EAP passwords), `SNMP` (SNMP communities, v3 auth/priv), `PASSWORD` (admin/manager/CLI passwords), `APITOKEN` (API tokens, OAuth credentials in configs), `CERT` (certificates), `KEY` (private keys, keytabs), `VPNPSK` (VPN/IPSec PSKs), `HOSTNAME` (device/AP names, FQDNs), `NAME` (operator-assigned names — site/org/vlan/subnet), `USER`/`EMAIL`/`PHONE` (user-identifying), `SERIAL`/`IMEI`/`IMSI`/`ICCID` (hardware), `IP` (non-public IPs — public DNS, RFC documentation, loopback are preserved verbatim).
- **NOT tokenized — pass through as cleartext** (refined in v2.3.1.1): MAC addresses (normalized to `aa:bb:cc:dd:ee:ff`); SSIDs (broadcast); all platform UUID `*_id` fields (`org_id`, `site_id`, `device_id`, `template_id`, etc. — they are already opaque); geographic data (`address`, `city`, `state`, `zip`, `latitude`, `longitude`, etc. — typically public on company websites). Treat these as you would any normal string value.
