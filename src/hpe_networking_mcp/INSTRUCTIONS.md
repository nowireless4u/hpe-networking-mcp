...
- **Translation Preview (read-only)**: central_translation_preview
  ...
  - Required `runtime_values` are translation-specific: `central_scope_id` always; `role_records` (full AOS 8 role list) for `central:policy` so the engine's preprocessing can compute role_attribution per ACL; `coa_servers` (the `aaa_prof.rfc3576_client[]` entries) for `central:auth_server` so co-located RFC 3576 / CoA servers fold onto the matching RADIUS server (issue #322); `cluster_strategy` (`ha_only` / `intent_site` / `intent_manual`) for `central:gateway_cluster`.
  - **AAA chain (AOS 8 → Central, all preview-only, Gateway-targeted to `MOBILITY_GW`):** the foundation profiles — `central:auth_server`, `central:server_group`, `central:captive_portal` — plus the gateway auth set — `central:dot1x_auth`, `central:mac_auth`, and the keystone `central:aaa_profile` (references the others by name). Translate in dependency order: auth-server / server-group / captive-portal / dot1x-auth / mac-auth first, then aaa-profile, then the consuming wlan/role objects. Write execution awaits the auth-server-secret PII tokenization PR.
