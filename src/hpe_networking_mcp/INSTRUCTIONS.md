...
- **Translation Preview (read-only)**: central_translation_preview
  ...
  - **WLAN SSID:** `central:wlan_ssid` (AOS 8 `virtual_ap` + joined `ssid_prof` → Central `wlan-ssids` + conditional `overlay-wlan` cluster binding). Runtime values: `target_mode` (the operator's per-SSID forward-mode decision — `bridged` / `tunneled` / `hybrid`; `bridged_and_tunneled` is v2), `ssid_profiles` (full `ssid_prof` list, joined by name), and `gw_cluster_list` (the clusters a tunneled/hybrid SSID binds to; an SSID may bind to several). Maps essid, `opmode` (AOS 8 → Central enum), forward-mode, and VLAN (`NAMED_VLAN` → `central:named_vlan`). v1 ships the SSID core; inline bridge-mode AAA, the essid-alias dual-profile case, radio/QoS, and the PSK secret are v2.
