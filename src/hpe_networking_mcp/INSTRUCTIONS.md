...
- **Common kinds:** `PSK` (WPA/SAE keys, passphrases, VRRP passphrases), `RAD` (RADIUS / RadSec shared secrets, EAP-tunneled passwords), `TACACS` (TACACS+ shared secrets, TACACS+-tunneled passwords), `SNMP` (SNMP communities, v3 auth/priv), `PASSWORD` (admin/manager/CLI passwords), `APITOKEN` (API tokens, OAuth credentials, **AWS-signed URLs**), `CERT` (certificates), `KEY` (private keys, keytabs), `VPNPSK` (VPN/IPSec PSKs), `HOSTNAME` (device/AP names, FQDNs, AAA server addresses), `NAME` (operator-assigned names — site/org/vlan/subnet), `USER`/`EMAIL`/`PHONE` (user-identifying), `SERIAL`/`IMEI`/`IMSI`/`ICCID` (hardware).
