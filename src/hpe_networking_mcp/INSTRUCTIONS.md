...
| *"visualize the X service"* / *"visualize the X auth service"* / *"visualize the X authentication service"* / *"visualize the X policy service"* / *"visualize the X ClearPass service"* / *"visualize the X policy"*, *"show me how X decides"*, *"show me the X service"*, *"walk the policy flow for X"*, *"walk the X service"*, *"render the X enforcement chain"*, *"render the X policy"*, *"draw the X service"*, *"draw the X policy"*, *"diagram X"*, *"diagram the X service"*, *"flowchart the X service"*, *"why does this MAC get this role"*, *"what does the X service look like end-to-end"* — any pairing of verbs **visualize / render / diagram / draw / flowchart / walk / show-me** with a ClearPass service name (including casual-sounding names like *"No Wireless For You"* or *"Guest Onboarding"* — treat the whole name as opaque and feed it through). Never guess what a named service does; call the tools. | `clearpass-policy-walker` |
...
- **Config-Model (v3.1.1.0+ bulk import — 389 tools across 19 modules)**: covers the rest of the Aruba Central configurable surface that wasn't already hand-curated. Every device-level config object reachable via `/network-config/v1alpha1/<segment>` now has a `central_get_<type>` and (where the spec supports it) a `central_manage_<type>` tool. Coverage areas:
  ...
  - **Payload shape**: every `central_manage_*` accepts `payload: dict`. Consult the Aruba Central config-model OpenAPI documentation, or call the corresponding `central_get_*` against an existing instance, to learn the field schema before authoring `create` payloads.
