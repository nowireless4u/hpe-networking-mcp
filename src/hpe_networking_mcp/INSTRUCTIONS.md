...
- **Translation Preview (read-only)**: central_translation_preview
  ...
  - **WLAN SSID:** `central:wlan_ssid` (AOS 8 `virtual_ap` + joined `ssid_prof` → Central `wlan-ssids` + conditional `overlay-wlan` cluster binding). Runtime values: `target_mode` (the operator's per-SSID forward-mode decision — `bridged` / `tunneled` / `hybrid` / `bridged_and_tunneled`), `ssid_profiles` (full `ssid_prof` list, joined by name), `gw_cluster_list` (the clusters a tunneled/hybrid/dual SSID binds to; an SSID may bind to several), and — for `bridged_and_tunneled` only — `bridge_scope_id` + `tunnel_scope_id` (the per-location split). Maps essid, `opmode` (AOS 8 → Central enum), forward-mode, and VLAN (`NAMED_VLAN` → `central:named_vlan`). `bridged_and_tunneled` emits an ESSID alias + a bridge profile + a tunnel profile (both sharing the alias) + the tunnel overlay — the same SSID in both modes across locations. Deferred: inline bridge-mode AAA, radio/QoS, and the PSK secret (PII-gated).
