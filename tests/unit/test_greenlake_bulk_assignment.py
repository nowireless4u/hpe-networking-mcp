...
class TestPatchAssignApplication:
    ...
    async def test_endpoint_uses_v2beta1_with_device_uuid_query_param(self) -> None:
        ...
        assert endpoint == "/devices/v2beta1/devices"
        params = call_kwargs.kwargs.get("params", {})
        assert params.get("id") == "test-uuid-123"
...
class TestSetupResumeStaleFilter:
    """setup_resume must filter stale cache entries not in the current valid_rows."""
    ...
    def _run(self, cache_data: dict, valid_rows=None):
        rows = valid_rows if valid_rows is not None else self._VALID_ROWS
        with patch(
            "hpe_networking_mcp.platforms.greenlake.tools._bulk_assignment._load_cache",
            return_value=cache_data,
        ):
            return setup_resume(None, "serialNumber,macAddress\nSN001,aa:bb:cc:dd:ee:01\n", rows)

    def test_stale_serial_excluded_from_returned_cache(self) -> None:
        """Serials not in valid_rows must not appear in the returned cache dict."""
        cache_data = {
            "SN001": {"status": "succeeded"},
            "OLD_REMOVED": {"status": "succeeded"},  # stale — not in current CSV
        }
        _, _, _, cache = self._run(cache_data)
        assert "OLD_REMOVED" not in cache
        assert "SN001" in cache

    def test_stale_succeeded_not_counted_in_skip(self) -> None:
        """A stale succeeded entry must not cause a non-existent row to be skipped."""
        cache_data = {
            "SN001": {"status": "succeeded"},
            "OLD_REMOVED": {"status": "succeeded"},
        }
        _, pending_rows, skipped_count, _ = self._run(cache_data)
        # Only SN001 is in valid_rows and it succeeded → 1 skipped, 1 pending (SN002)
        assert skipped_count == 1
        assert len(pending_rows) == 1
        assert pending_rows[0]["serialNumber"] == "SN002"

    def test_all_current_succeeded_cache_cleared(self) -> None:
        """When all current rows succeed and no stale entries, returned cache matches."""
        cache_data = {
            "SN001": {"status": "succeeded"},
            "SN002": {"status": "succeeded"},
        }
        _, pending_rows, skipped_count, cache = self._run(cache_data)
        assert pending_rows == []
        assert skipped_count == 2
        assert set(cache) == {"SN001", "SN002"}
