...
class TestColumnAliasResolution:
    ...
    def test_sub_key_alias_maps_to_subscription_key(self) -> None:
        """sub_key must resolve to subscriptionKey (normalize_header strips underscores)."""
        csv_text = "serialNumber,macAddress,sub_key\nABC123,11:22:33:44:55:66,MYKEY\n"
        result = parse_csv(None, csv_text)
        assert result.error is None
        row = result.valid_rows[0]
        assert "subscriptionKey" in row, f"Expected subscriptionKey, got keys: {list(row)}"
        assert "sub_key" not in row

    def test_location_alias_case_insensitive(self) -> None:
        """Location, LOCATION, and location must all resolve to 'location'."""
        for header in ("Location", "LOCATION", "location"):
            csv_text = f"serialNumber,macAddress,{header}\nABC123,11:22:33:44:55:66,Building A\n"
            result = parse_csv(None, csv_text)
            assert result.error is None, f"Unexpected error for header {header!r}"
            row = result.valid_rows[0]
            assert "location" in row, f"Header {header!r}: expected 'location' key, got {list(row)}"
            assert header not in row or header == "location"

    def test_tags_alias_case_insensitive(self) -> None:
        """Tags, TAGS, and tags must all resolve to 'tags'."""
        for header in ("Tags", "TAGS", "tags"):
            csv_text = f"serialNumber,macAddress,{header}\nABC123,11:22:33:44:55:66,prod\n"
            result = parse_csv(None, csv_text)
            assert result.error is None, f"Unexpected error for header {header!r}"
            row = result.valid_rows[0]
            assert "tags" in row, f"Header {header!r}: expected 'tags' key, got {list(row)}"

    def test_region_alias_case_insensitive(self) -> None:
        """Region, REGION, and region must all resolve to 'region'."""
        for header in ("Region", "REGION", "region"):
            csv_text = f"serialNumber,macAddress,{header}\nABC123,11:22:33:44:55:66,us-west-2\n"
            result = parse_csv(None, csv_text)
            assert result.error is None, f"Unexpected error for header {header!r}"
            row = result.valid_rows[0]
            assert "region" in row, f"Header {header!r}: expected 'region' key, got {list(row)}"
