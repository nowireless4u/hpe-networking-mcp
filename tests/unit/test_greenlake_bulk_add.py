...
class TestGreenlakeBulkAddDevices:
    ...
    async def test_pii_gate_unacknowledged_returns_string(self, mock_ctx: MagicMock) -> None:
        ...
        assert "acknowledged=True" in result
        assert "serial number" in result.lower() or "serial numbers" in result.lower()

    async def test_pii_gate_blocks_parse_csv(self, mock_ctx: MagicMock) -> None:
        with patch("hpe_networking_mcp.platforms.greenlake.tools.bulk_add.parse_csv") as mock_parse:
    ...
    async def test_pii_gate_blocks_api_calls(self, mock_ctx: MagicMock) -> None:
        await greenlake_bulk_add_devices(
            mock_ctx,
            acknowledged=False,
        )
        assert mock_ctx.lifespan_context["greenlake_token_manager"].get_auth_headers.call_count == 0
    ...
    async def test_batch_failure_continues(self, mock_ctx: MagicMock, tmp_path: pathlib.Path) -> None:
        ...
        with (
            ...
            patch(
                "hpe_networking_mcp.platforms.greenlake.tools.bulk_add._device_add_limiter",
            ) as mock_limiter,
        ):
...
class TestBulkAddPhase20Integration:
    ...
    async def _run_tool(
        ...
        with (
            ...
            patch(
                "hpe_networking_mcp.platforms.greenlake.tools.bulk_add._device_add_limiter",
            ) as mock_device_limiter,
            patch(
                "hpe_networking_mcp.platforms.greenlake.tools._bulk_assignment._subscription_patch_limiter",
                mock_sub_limiter,
            ),
            patch(
            ...
            mock_device_limiter.__aenter__ = AsyncMock(return_value=None)
            mock_device_limiter.__aexit__ = AsyncMock(return_value=False)
            result = await greenlake_bulk_add_devices(
                mock_ctx,
                acknowledged=True,
                csv_path=str(csv_file),
    ...
    async def test_subscription_patch_limiter_acquired_for_each_patch(
        ...
        """SUB-02: _subscription_patch_limiter is acquired for EVERY assignment PATCH."""
        sn = "SN001"
        csv_file = _write_csv(
            tmp_path,
            [
        ...
        # 1 row with both service and subscription → 2 PATCH calls → 2 limiter acquisitions
        assert mock_sub_limiter.__aenter__.call_count == 2, (
            f"Expected 2 limiter acquisitions, got {mock_sub_limiter.__aenter__.call_count}"
        )

    async def test_separate_limiter_instances(self) -> None:
        """_subscription_patch_limiter must be a distinct instance from _device_add_limiter."""
        from hpe_networking_mcp.platforms.greenlake.tools._bulk_assignment import (
            _subscription_patch_limiter,
        )
        from hpe_networking_mcp.platforms.greenlake.tools.bulk_add import _device_add_limiter

        assert _subscription_patch_limiter is not _device_add_limiter
    ...
    async def test_patch_endpoints_use_v2beta1(self, mock_ctx: MagicMock, tmp_path: pathlib.Path) -> None:
        ...
        for call in calls:
            ...
            assert endpoint.startswith("/devices/v2beta1/devices?id="), (
                f"Endpoint does not start with /devices/v2beta1/devices?id=: {endpoint!r}"
            )
