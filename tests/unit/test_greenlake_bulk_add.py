...
class TestGreenlakeBulkAddDevices:
    # --- Write gate tests (ENABLE_GREENLAKE_WRITE_TOOLS / confirm_write) ---

    async def test_write_gate_declined_returns_status_dict(self, mock_ctx: MagicMock) -> None:
        """confirm_write decline → tool returns a status dict, no API calls."""
        from fastmcp.server.elicitation import DeclinedElicitation

        declined_result = {"status": "declined", "message": "Action declined by user."}
        with patch(
            "hpe_networking_mcp.platforms.greenlake.tools.bulk_add.confirm_write",
            new=AsyncMock(return_value=declined_result),
        ):
