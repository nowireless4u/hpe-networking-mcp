...
(Juniper Apstra), `axis_*` (Axis Atmos Cloud), `aos8_*` (Aruba OS 8),
`uxi_*` (Aruba UXI), and `edgeconnect_*` (Aruba EdgeConnect / Silver Peak Orchestrator).
...
- **`execute(code)`** — run async Python in a sandbox; `await call_tool(name, params)` is available in scope and dispatches to any of the 4109 underlying tools, and `await report_progress(progress, total=None, message=None)` streams status to the client during long multi-call blocks
...
All 4109 per-platform tools documented below still exist and are reachable via `await call_tool(name, params)` inside `execute()`. The per-platform sections below serve as the **full tool index** — humans read them directly; the AI discovers them via the discovery tools (`tags`, `search`, `get_schema`).
...
Set `MCP_TOOL_MODE=dynamic` to use the v2.x meta-tool surface (per-platform discovery — see next section). The `static` mode was REMOVED in v3.0.0.0 — at 4109 tools / ~64K tokens it was no longer practical.
...
The 4109 per-platform tools are reachable via `<platform>_invoke_tool(name=..., arguments={...})`. Best when an orchestrator wants explicit per-tool dispatch rather than the sandboxed Python composition that code mode provides.
...
|---|---|---|
| 1 | `tags(detail="brief")` | Browse the tag space — returns platform buckets (`mist (1050 tools)`, `central (90)`, `axis (25)`, etc.) plus module categories |
...
The `static` mode was REMOVED in v3.0.0.0 — at 4109 tools / ~64K tokens it was no longer practical. Setting `MCP_TOOL_MODE=static` raises ValueError at startup.
...
| HPE GreenLake | 495 | 424 | -- | 919 |
| Axis Atmos Cloud | 12 | 13 | -- | 25 |
| Aruba OS 8 | 26 | 12 | 9 | 47 |
| HPE UXI | 11 | 10 | -- | 21 |
| Aruba EdgeConnect | 660 | 556 | -- | 1216 |
...
## Aruba Central (669 tools + 12 prompts)
...
> Returns a lightweight list of all sites with IDs, health scores, and counts.

No parameters. Returns `{"items": [...]}` (#491) — a list of site objects (each with `site_name`, `site_id`, `health`, `total_devices`, `total_clients`, `total_alerts`), sorted by health score (worst first).
...
> Triggers a full configuration sync (resync) for one or more devices — the standard remediation when a device is `OUT_OF_SYNC` or shows `CONFIG_PUSH_FAILURES`. Idempotent and non-destructive (re-pushes the *intended* config; does not change it). `OPERATIONAL` — runs immediately, no confirmation prompt, not gated behind `ENABLE_CENTRAL_WRITE_TOOLS`. Central silently skips serials it can't act on and reports how many were actually triggered.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| serials | list[str] | Yes | Device serial numbers to resync (non-empty list). |
...
## HPE GreenLake (919 tools — spec-generated)
...
| subscription_key | str | No | Subscription key applied to **every** device lacking a `subscriptionKey` column (per-row column wins). |
| service_id | str | No | Service/application ID applied to **every** device lacking a `serviceId` column (per-row column wins). |
| location | str | No | Location ID applied to **every** device lacking a `location` column (per-row column wins). |
| tags | str | No | Tags applied to **every** device lacking a `tags` column (per-row column wins). |

Mandatory CSV columns: `serialNumber` (aliases: `serial`, `sn`, `serial_number`) and `macAddress` (aliases: `mac`, `mac_address`). Optional per-row columns: `partNumber`, `serviceId` (alias `service`), `subscriptionKey`, `location`, `tags`. The `subscription_key` / `service_id` / `location` / `tags` **parameters** apply the same value to the whole batch (any row missing that column) — the way an uploaded serial/MAC list gets subscribed + assigned without the AI editing the file. The `greenlake-device-onboarding` skill is the guided front-end for this.
