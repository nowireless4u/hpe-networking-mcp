...
| `central-qos-policy` | End-to-end runbook for switch QoS in Aruba Central, covering both halves: **ingress / marking** — traffic classes (`named-condition`) → marker policy (`policy` with `type: POLICY_QOS`) → VLAN-interface bind (creates the VLAN if missing); and **egress / system-wide** — queue-profile (`qos-queue`) → schedule-profile (`qos-schedule`) → atomic apply + DSCP-map (`qos-global`, one named library object holding `q-profile`+`sched-profile` refs + `dscp-map[]`) → scope assignment to activate on devices. Carries exact live-verified payload shapes — the `ADDRESS_SUBNET_MASK` discriminator + dotted-mask format, the undocumented `condition.named-condition.condition-reference`, `ACTION_QOS` + `local-queue-priority`/`dscp`, `access-group-vlan-in` routed-in bind, the `WEIGHTED` algorithm enum that collapses both DWRR and WFQ, the `"DEFAULT"` enum literal for factory-default refs (not the string `"factory-default"`), and `qos-global` PATCH semantics (field-preserve + array-upsert-by-key, so `dscp-map` survives a ref flip). Section G is an **automated update-applied-profile block** that handles the API's reject-edits-while-applied rule: detect-if-applied → flip qos-global to `DEFAULT`/`DEFAULT` → edit → restore refs with read-back assertions and always-rollback. Read-back-and-assert after every write (the API returns success while silently dropping under-specified subtrees). CX vs AOS-S (PVOS) aware. | v3.2.2.0 / v3.2.3.1 / v3.2.3.3 |
