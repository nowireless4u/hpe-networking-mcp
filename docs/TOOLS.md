...
| `central-site-dashboard` | Single-site operational dashboard for Aruba Central — overall health score, device status by type, client counts, and active alerts, rendered through `generate_prefab_ui` (Generative UI). The fast path for "build me a dashboard for site X": a fixed 3-4 call gather with the exact response-envelope unwraps baked in (site_health `data` is a **list** → `[0]`; devices is a **list or the "No devices" string**; alerts rows are under `data["items"]`, not `result`; `central_get_alerts` requires `site_id`), replacing the ~20+ exploratory round-trips ad-hoc dashboard building burns rediscovering schemas and fighting the envelope. Always emits a text walkthrough beneath the board so it stays useful in non-MCP-Apps clients where the widget no-ops. Central-only / single-site by design; cross-platform + ClearPass overlay are deferred follow-ups. Read-only. | v3.3.13.5 |
