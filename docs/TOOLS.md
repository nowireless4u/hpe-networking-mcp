...
| `central-qos-policy` | Schema-first runbook for building switch QoS into Aruba Central: traffic classes (`named-condition`) → marker/classifier policy (`policy` with `type: POLICY_QOS`, per-rule `packet-marking.dscp` + `local-queue-priority`) → interface/VLAN bind. CX vs AOS-S (PVOS) aware. Reads the distilled `payload_schema` from `central_get_tool_schema` so field names/enums are never guessed; validates one object then bulk-pushes. | v3.2.2.0 |
