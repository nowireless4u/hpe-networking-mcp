...
| `central-qos-policy` | Runbook for building switch QoS into Aruba Central: traffic classes (`named-condition`) → marker policy (`policy` with `type: POLICY_QOS`) → VLAN-interface bind (creates the VLAN if missing). Carries exact live-verified payload shapes — the `ADDRESS_SUBNET_MASK` discriminator + dotted-mask format, the undocumented `condition.named-condition.condition-reference`, `ACTION_QOS` + `local-queue-priority`/`dscp`, `access-group-vlan-in` routed-in bind — since several fields are absent from the spec. Read-back-and-assert after every write (the API returns success while silently dropping under-specified subtrees). CX vs AOS-S (PVOS) aware. | v3.2.2.0 / v3.2.3.1 |
